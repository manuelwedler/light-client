  ...
  return timer(pollingInterval, httpTimeout).pipe(
    mergeMap(() =>
      defer(async () => {
        ...
        catchError((err) => {
          // if peer is already in the room, give up retry
          if (matchError('is already in the room', err)) return throwError(err);
          // otherwise, log and suppress to retry on next timer emit
